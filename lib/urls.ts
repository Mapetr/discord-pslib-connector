...
export function getDiscordLoginURL(origin: string): string {
  ...
  return `https://discord.com/oauth2/authorize?response_type=code&client_id=${client_id}&scope=identify%20guild.join&redirect_uri=${encodeURIComponent(origin)}%2Fdiscord&prompt=consent`
}
